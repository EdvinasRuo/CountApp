...
const mapDispatchToProps =(dispatch) => {
    return {
        deleteFromBudget: (id) => {dispatch(deleteFromBudget(id))},
    };
}; 
...
const budgetScreen = ({navigation, deleteFromBudget, budget}) => {
    ...
    <>  
        <Spacer>
            <Text h3 style={styles.title}>Add budget for current month.</Text>  
        </Spacer>    
        <Spacer>
            ...
            ?   <FlatList 
                    ...
                    renderItem = {(item) => {
                        ...
                        <View style={styles.listContainer}>
                            <TouchableOpacity onPress={()=> deleteFromBudget(item.item.id)}>
                ...
                /> 
            : <Text style={styles.smallInfo}>There are no budget items yet. Press plus below to add.</Text>}
        </Spacer>
        <Spacer>
