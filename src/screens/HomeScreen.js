...
const homeScreen = ({ navigation, monthsExpenses, fixedExpenses }) => {
  ...
  thisMonthsFixed = fixedExpenses.filter(obj => {
    return (
      (moment().format('YYYY') === obj.year &&
        obj.month <= moment().format('M')) ||
      (moment().format('YYYY') === obj.year &&
        obj.deleteMonth > moment().format('M')) ||
  ...
  return (
    <ScrollView>
      <Spacer>
        ...
        <Text style={styles.quickMeniu}>Your quick - meniu items: </Text>
      </Spacer>
      <QuickList />
      <Spacer>
      ...
      <Spacer>
        ...
        {thisMonthsArray.length > 0 ? (
          ...
          <Text style={styles.smallInfo}>
            No expenses yet. Press plus sign above to add.
          </Text>
        )}
      </Spacer>
      <Spacer>
        ...
        {thisMonthsFixed.length > 0 ? (
          ...
          <Text style={styles.smallInfo}>
            No fixed expenses yet. Press 'Fixed Expenses' in the meniu below to
            add.
          </Text>
        )}
      </Spacer>
