    ...
    const totalSorted = uniqueYears.map((year) => {
      ...
      const sortedByMOnth =  existingMonths.map((month) => {
            ...
            allCosts.map((expense) => {
                ...
                if((!expense.fixed && (expense.month === month && expense.year === year))
                // Fixed, same year, earlier months, no delete month
                || (expense.fixed && expense.year === year && expense.month <= month && !expense.deleteMonth)
                // Fixed, same year delete month before listing month  
                || (expense.fixed && expense.year === year && expense.deleteMonth > month) 
                //Fixed, earlier year, no delete date 
                || (expense.fixed && expense.fixed && expense.year < year && !expense.deleteMonth)) {
            ...
            return {year: year,
                    month: month,
                    expenses:uniqueItems
            }
        });

        return { year: year, 
                expenses: sortedByMOnth
        }
